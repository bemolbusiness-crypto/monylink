...
function TransferSimulator() {
  const [amount, setAmount] = useState(150000) // FCFA
  // MonyLink : taux client 1€ = 700 FCFA → on divise par 700
  const mlEuros = Math.round(amount / 700)
  // WU : frais tout compris ~13% (frais de transfert + taux dégradé sur corridor Cameroun-France)
  // puis conversion au taux interbancaire officiel fixe XAF/EUR = 655.957
  const wuEuros = Math.round((amount * 0.87) / 655.96)
  const savings = mlEuros - wuEuros
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {/* Comparison EUR reçus */}
      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', textAlign: 'center', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1 }}>Le destinataire reçoit en €</div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <div style={{ background: 'rgba(34,211,176,0.07)', border: '1px solid rgba(34,211,176,0.2)', borderRadius: 12, padding: '12px 14px' }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: '#22D3B0' }}>{fmt(mlEuros)} €</div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '12px 14px', opacity: 0.65 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 800, color: 'rgba(255,255,255,0.5)' }}>{fmt(wuEuros)} €</div>
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 3 }}>Frais ~13% tout compris</div>
        </div>
      </div>
      {savings > 0 && (
        <div style={{ background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.25)', borderRadius: 10, padding: '10px 14px', textAlign: 'center' }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#F97316' }}>Tu économises <strong style={{ fontSize: 16 }}>{fmt(savings)} €</strong> avec MonyLink 🎉</span>
        </div>
      )}
      ...
      <section className="sec" style={{ padding: '0 80px 80px', maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1, boxSizing: 'border-box', width: '100%' }}>
        <div className="sim-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center' }}>
          <div className="sim-text">
            ...
            <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.45)', lineHeight: 1.7, marginBottom: 24 }}>Pour 150 000 FCFA envoyés, tu reçois <strong style={{ color: '#fff' }}>214€</strong> avec MonyLink contre ~199€ chez Western Union. Taux fixe, zéro frais caché.</p>
      ...
      <section id="pricing" className="sec" style={{ padding: '0 80px 80px', maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <div className="pricing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 32 }}>
          {[
            ...
            { label: 'Western Union', rate: '1€ = 655 FCFA', fee: 'Frais ~13% tout compris', total: '~199 €', highlight: false },
            { label: 'Virement bancaire', rate: '1€ = 655 FCFA', fee: '+ frais fixes ~15€', total: '~184 €', highlight: false },
          ].map(p => (
        ...
        <div style={{ background: 'rgba(34,211,176,0.06)', border: '1px solid rgba(34,211,176,0.15)', borderRadius: 16, padding: '16px 24px', textAlign: 'center', fontSize: 14, color: 'rgba(255,255,255,0.6)' }}>
          💡 <strong style={{ color: '#22D3B0' }}>Exemple concret :</strong> Pour 150 000 FCFA envoyés depuis Douala — MonyLink donne <strong style={{ color: '#fff' }}>15€ de plus</strong> au destinataire que Western Union.
        </div>
      </section>
