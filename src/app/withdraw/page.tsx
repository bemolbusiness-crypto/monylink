    ...
    <div style={{ maxWidth: 430, margin: '0 auto', minHeight: '100dvh', background: 'var(--bg)', paddingBottom: 80 }}>
      ...
      <div style={{ background: 'linear-gradient(160deg,var(--bg) 0%,#1a0d35 60%,var(--bg2) 100%)', padding: '48px 20px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          <button onClick={() => step === 'amount' ? router.back() : setStep(step === 'rib' ? 'amount' : step === 'confirm' ? 'rib' : 'amount')}
          ...
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 800 }}>Retirer vers mon RIB</h1>
            <div style={{ fontSize: 12, color: 'var(--w40)', marginTop: 2 }}>Solde : <strong style={{ color: 'var(--teal)' }}>{formatCurrency(balance,'EUR')}</strong></div>
          </div>
        </div>
